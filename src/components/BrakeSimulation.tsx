...
const BrakeSimulation: React.FC<Props> = ({ locale }) => {
  ...
  const setup = (p5: any, parentRef: Element) => {
    p5.createCanvas(900, 250).parent(parentRef);
  };
  ...
  const draw = (p5: any) => {
    ...
    p5.fill(80);
    p5.rect(-scrollX.current, 150, (maxDist + 500) * ppm, 100);
    ...
    if (brakeAtRef.current !== null) {
      ...
      p5.textSize(16);
      p5.fill("red");
      p5.text(t.brakeStart, startX - 30, 125);
    }

    p5.fill(0, 150, 250);
    p5.rect(carX, 150 - carH / 2, carW, carH);

    if (ended && isBraking && speedRef.current === 0) {
      p5.fill("green");
      p5.triangle(carX, 140, carX - 5, 130, carX + 5, 130);
      p5.textSize(16);
      p5.fill("green");
      p5.text(t.carStopped, carX - 30, 125);
    }

    p5.fill(0);
    p5.textSize(13);
    p5.text(
      `${t.speed}: ${speedRef.current.toFixed(2)} ${t.unit.ms} (${(
        speedRef.current * 3.6
      ).toFixed(1)} ${t.unit.kmh})`,
      10,
      20
    );
    ...
    p5.text(
      `${t.brakingDist}: ${
        isBraking
          ? (posRef.current - (brakeAtRef.current || 0)).toFixed(2)
          : calcBrakingDist.toFixed(2)
      } ${t.unit.m}`,
      10,
      100
    );
